...
function handleFileUpload(request, response) {
    ...
    const status = []; // This will hold the status for the renaming of each file
    ...
    request.files.forEach(file => {
        ...
        fs.rename(savedPath, finalPath, (err) => {
            if (err) {
                console.error('Error renaming file', err);
                status.push({file: originalName, error: err.message});
            } else {
                console.log(`Renamed ${finalPath} to its original name`);
                status.push({file: originalName, savedAs: finalPath});
            }
        });
    });
